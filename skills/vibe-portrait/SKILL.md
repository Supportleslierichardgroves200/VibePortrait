...
For each dimension, extract signals from the sampled messages:

### 2.1 Communication Style
- **Language detection (for i18n)**: if >50% of sampled messages are in Chinese, set `meta.lang = "zh"`. Otherwise `"en"`. This determines the portrait page language — all fixed UI text will render in the detected language.
- Language distribution (Chinese / English / other / mixed)
- Directness level: ratio of imperative commands vs conversational messages
...
- Count unique tools, frameworks, and languages referenced
- Classify domains: Web, ML/AI, 3D/Graphics, Systems/Infra, Agent/LLM, Mobile, Data, Embedded/Robotics

### 2.3 Technical Depth
- Ratio of architecture/design discussions vs implementation/how-to questions
- Evidence of multi-step reasoning and system design
- Debugging sophistication (hypotheses vs "it doesn't work")
- Evidence of building novel systems

### 2.4 Decision Patterns
- Pivot speed: how quickly they abandon failing approaches
- Evidence-based decisions: do they ask for data before deciding?
- Sunk cost resistance: willingness to revert and start over
- Delegation clarity: how precisely they specify requirements

### 2.5 Collaboration Style
- Trust level: how much they verify AI output
- Feedback granularity: specific corrections vs vague complaints
- Praise frequency
- Autonomy expectation: step-by-step approval vs "just get it done"

### 2.6 Work Rhythm
- Session patterns from timestamp gaps (if available)
- Task switching frequency
- Project diversity (count distinct project references)
- Sprint vs marathon patterns

**For each dimension, produce:**
- A score (0-100)
- A one-line summary
- 2-3 evidence quotes from their actual messages (keep quotes short, anonymize API keys/passwords/URLs)
...
### Loading community personas

Users can download other people's persona skills and place them in:
```
~/.claude/skills/vibe-portrait-personas/{person-id}/SKILL.md
```

Then invoke with: "think like {person-id}" or "像{person-id}一样思考这个问题"

### Directory layout
```
~/.claude/skills/vibe-portrait-personas/
├── me/                  # The user's own persona (auto-generated)
│   └── SKILL.md
├── linus-torvalds/      # Example community persona
│   └── SKILL.md
└── zhuge-liang/         # Example community persona
    └── SKILL.md
```

The `me/` directory is always the user's own persona. All other directories are imported community personas. This separation is intentional and must not be changed.
